...
class RepairLoop:
    def __init__(self, settings, admin, candidates, ledger, gate_case, emit):
    ...
    async def run(self, program, inputs, environment, session):
        ...
        remaining = (
            ...
            "Create the requested invoice and mark it paid. Do not repeat login or earlier writes. "
        ...
        gate = await GateRunner(
            self.settings.runs_dir / "gates", self.candidates, self.gate_case, self.emit
        )(request)
        row = await self.ledger.promote(
            gate, program.content_hash(), "restoration", [c.case_id for c in suite()]
        )
